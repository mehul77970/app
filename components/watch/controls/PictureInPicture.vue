...
<template>
  <TooltipProvider v-if="!pipAPIAvaliable">
    <Tooltip>
      <TooltipTrigger class="cursor-not-allowed">
        <PhPictureInPicture :size="24" class="text-red-400 m-1.5" />
      </TooltipTrigger>
  ...
  <Button v-else variant="ghost" size="icon" @click="togglePiP">
    <PhPictureInPicture :size="24" :weight="inPiP ? 'fill' : 'regular'" />
  </Button>
</template>
