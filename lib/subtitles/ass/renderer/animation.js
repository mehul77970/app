...
import { rotateTags, scaleTags, skewTags } from "./transform.js";
...
export function createMove(move, duration) {
  ...
  const keyframes = [
    ...
    t2 > 0 && t2 < duration
      ? { offset: t2 / moveDuration, transform: end }
      : null,
...
export function createFadeList(fade, duration) {
  ...
  const keyframes = offsets.map((t, i) => ({
    offset: t,
...
function getEasing(duration, accel) {
  ...
  const points = Array.from({ length: frames + 1 }).map(
    (_, i) => (i / frames) ** accel,
  );
...
function createTagKeyframes(fromTag, tag, key) {
  ...
  if (key === "a3" || key === "c3") {
    return [
      [
        "border-color",
        color2rgba((tag.a3 || fromTag.a3) + (tag.c3 || fromTag.c3)),
      ],
    ];
  }
  if (key === "a4" || key === "c4") {
    return [
      [
        "shadow-color",
        color2rgba((tag.a4 || fromTag.a4) + (tag.c4 || fromTag.c4)),
      ],
    ];
  }
  if (key === "fs") {
...
export function createClipAnimations(el, dialogue, store) {
  return dialogue.slices
    ...
    .map(({ t1, t2, accel, tag }) => {
      const keyframe = {
        offset: 1,
        clipPath: createRectClip(
          tag.clip,
          store.scriptRes.width,
          store.scriptRes.height,
        ),
      };
      const duration = Math.max(0, t2 - t1);
      return initAnimation(el, [keyframe], {
