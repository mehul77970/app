export function setEffect(dialogue, store) {
  ...
  const { name, y1, y2, leftToRight, fadeAwayWidth, fadeAwayHeight }
    = dialogue.effect
  ...
  if (name === 'banner') {
    ...
    $area.style.justifyContent = ['flex-end', 'center', 'flex-start'][
      dialogue.align.v
    ]
  }
  if (name.startsWith('scroll')) {
    ...
    $area.style.justifyContent = ['flex-start', 'center', 'flex-end'][
      dialogue.align.h
    ]
  }
  if (fadeAwayHeight) {
